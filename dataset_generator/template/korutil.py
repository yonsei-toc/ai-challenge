...
def append_connection(target, type):
    ...
    if is_hangul_char(target[-1]):
        return target + tbl[type][0 if has_last_syllable(target) else 1]
    else:
        if target[0] == '(':
            # (가), etc.
            return target + tbl[type][0 if has_last_syllable(target[-2]) else 1]
        else:
            return target + tbl[type][random.randint(0, 1)]
