...
@gen.problems.register
def prob4_01_01max(selector, tokenpool, clskey):
    '''
    - 43, 92, 71, 64가 있습니다. 그중에서 가장 큰 수는 얼마입니까?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    nums_len = random.randint(2, 5)
    nums_k = tokenpool.sample(range(0, 100), nums_len)
...
@gen.problems.register
def prob4_01_01min(selector, tokenpool, clskey):
    '''
...
@gen.problems.register
def prob4_01_02max(selector, tokenpool, clskey):
    '''
    - 은지, 태형, 유나, ..은 각각 사과 43, 92, 71, 64를 가지고 있습니다. 가장 많은 사과를 가진사람의 개수는?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    item = selector.get(clskey.fruit)
    nums_len = random.randint(2, 5)
...
@gen.problems.register
def prob4_01_02min(selector, tokenpool, clskey):
    '''
    - 은지, 태형, 유나, ..은 각각 사과 43, 92, 71, 64를 가지고 있습니다. 가장 적은 사과를 가진사람의 개수는?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    item = selector.get(clskey.fruit)
    nums_len = random.randint(2, 5)
...
@gen.problems.register
def prob4_01_02avg(selector, tokenpool, clskey):
    '''
    - 은지, 태형, 유나, ..은 각각 사과 43, 92, 71, 64를 가지고 있습니다. 가장 적은 사과를 가진사람의 개수는?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    ...
    # syntactic randomize
    ques_trailing = random.choice(['가지고 있습니까?', '가졌는지 구하시오.', '인가?'])
...
@gen.problems.register
def prob4_01_03max(selector, tokenpool, clskey):
    '''
    - 은지, 태형, 유나, ..은 각각 주스를 43l, 92l, 71l, 64l 마셨습니다. 가장 많은?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    item = selector.get(clskey.drink)
    nums_len = random.randint(2, 5)
    ...
    # syntactic randomize
    ques_trailing = random.choice(['를 마셨습니까?', '인지 구하시오.', '를 마셨는지 구하시오.'])
...
@gen.problems.register
def prob4_01_03min(selector, tokenpool, clskey):
    '''
    - 은지, 태형, 유나, ..은 각각 주스를 43l, 92l, 71l, 64l 마셨습니다. 가장 적은?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    item = selector.get(clskey.drink)
    nums_len = random.randint(2, 5)
...
@gen.problems.register
def prob4_01_03avg(selector, tokenpool, clskey):
    '''
    - 은지, 태형, 유나, ..은 각각 주스를 43l, 92l, 71l, 64l 마셨습니다. 평균?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    item = selector.get(clskey.drink)
    nums_len = random.randint(2, 5)
...
@gen.problems.register
def prob4_01_04max(selector, tokenpool, clskey):
    '''
    - 상자에 사탕이 각각 43, 92, 71, 64 개 들어있습니다. 사탕이 가장 많이 담긴 상자의 사탕수는 ?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    container = selector.get(clskey.container)
    item = selector.get(clskey.flower)
...
@gen.problems.register
def prob4_01_05min(selector, tokenpool, clskey):
    '''
...
@gen.problems.register
def prob4_01_06max(selector, tokenpool, clskey):
    '''
    - clskey.race에서 4명의 학생은 각각 ..점..점..점을 얻었습니다. 가장 높은 점수는?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    l = random.randint(2, 5)
    race = selector.get(clskey.race)
    ...
    # syntactic randomize
    ques_trailing = random.choice(['는 무엇입니까?', '를 구하시오.'])
...
@gen.problems.register
def prob4_01_06min(selector, tokenpool, clskey):
    '''
    - clskey.race에서 4명의 학생은 각각 ..점..점..점을 얻었습니다. 가장 낮은 점수는?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    l = random.randint(2, 5)
    race = selector.get(clskey.race)
    ...
    nums_k = tokenpool.sample(range(0, 100), l)
    end = random.choice(['꼴찌의', '꼴등의', '최하위', '마지막 등수의', '가장 낮은'])

    # syntactic randomize
    ques_trailing = random.choice(['는 얼마입니까?', '를 구하시오.'])
...
@gen.problems.register
def prob4_01_06avg(selector, tokenpool, clskey):
    '''
    - clskey.race에서 4명의 학생은 각각 ..점..점..점을 얻었습니다. 가장 평균 점수는?
    '''
    # this may be a meta-token (e.g., #1), referring a number.
    l = random.randint(2, 5)
    race = selector.get(clskey.race)
