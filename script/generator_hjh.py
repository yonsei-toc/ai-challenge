...
def c5p1(sel, tokenpool, clskey):
    ...
    return gen.build(
        ...
        question = '{name1}{#와} {name2}의 합을 구하시오.',
        equation = gen.EqnRef('sum',num1_token, num2_token, num3_token, num4_token),
...
def c5p2(sel,tokenpool,clskey):
    ...
    return gen.build(    
        body = '서로 다른 두 수 {name1}, {name2}{#이} 있습니다.',
        question = '두 자리 수끼리의 뻴셈식 {num1}{name1}-{name2}{num2}={num3}{num4}에서 {name1}{#와} {name2}의 합을 구하시오.',
        equation = gen.EqnRef('c5p2',num1_token,num2_token,num3_token,num4_token),
...
def c5p3(sel,tokenpool,clskey):
    ...
    return gen.build(
        body = '서로 다른 네 수 {name1}, {name2}, {name3}, {name4}{#가} 있습니다.',
        question = '세 자리 수끼리의 덧셈식 {num1}{name1}{num2}+{name2}{num3}{name3}={name4}{num4}{num5}에서 {name4}{#를} 구하시오.',
        equation = gen.EqnRef('c5p3', num1_token, num2_token, num3_token, num4_token, num5_token),
...
def c5p4(sel,tokenpool,clskey):
    ...
    return gen.build(
        ...
        question = '나머지 {name2}가 가장 큰 수일 때 {name1}{#를} 구하시오.',
        equation = gen.EqnRef('c5p4',num1,num2,name2),
...
def c5p5(sel,tokenpool,clskey):
    ...
    return gen.build(
        body = '네 자리 수 {num1}{name1}{num2}{num3}{#를} 백의 자리에서 반올림하면 {num4}{#이} 됩니다.',
        question = '{num5}부터 {num6}까지의 숫자 중 {name1}에 쓸 수 있는 숫자는 {total}몇 개{ques_trailing}',
        equation = gen.EqnRef('c5p5', num1_token, num4_token, num5_token, num6_token),
...
def c6p5(sel,tokenpool,clskey):
    ...
    return gen.build(
    body = '{num1}에 {name1}{#를} 곱해야 하는데 잘못하여 {name1}{#를} {num2}{#으?}로 나누었더니 {num3}{#이} 되었습니다.',
    question = '바르게 계산한 결과를 구하시오.',
    equation = gen.EqnRef('c6p5',num1_token,num2_token,num3_token),
...
def c7p5(sel,tokenpool,clskey):
    ...
    return gen.build(
        ...
        '{name3}{#는} {num1}년 후에 {num2}살이 되고, {name3}{#이?}는 올해 {num3}살입니다.'
        ]),
        question = '{name_num}명 중에서 나이가 {num4}번째로 적은 사람은 누구{ques_trailing}',
        equation = gen.EqnRef('c7p5', name1_token,name2_token,name3_token,name4_token,name5_token,num4_token),
...
def c8p5(sel,tokenpool,clskey):
    ...
    return gen.build(
        body = '둘레가 {num1}인 직사각형이 있습니다.',
        question = '이 직사각형의 {edge1} 길이가 {edge2} 길이의 {num2}배일 때 {edge1}는 몇 {unit}입니까?',
        equation = gen.EqnRef('c8p5',edge1_token,edge2_token,num1_token,num2_token),
